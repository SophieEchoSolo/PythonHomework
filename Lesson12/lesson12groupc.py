wordList = ['Four','score','and','seven','years','ago','our','fathers','brought','forth','on',
...
print('Capitalize each word in the list')
newList = [word.capitalize() for word in wordList ]
print(newList)
...
for word in wordList:
    if len(word) == 5:
        len5List.append(word)
print(len5List)

firstletter = []
fletter = input("Which first letter would you like to find?: ")
for word in wordList:
    if word.lower()[0]==fletter:
        firstletter.append(word)
print(f"The following list shows all words starting with the letter {fletter}: \n")
print(firstletter)

allCapList = []
for word in wordList:
    word = word.upper()
    allCapList.append(word)
print(allCapList)

containsLetter = []
findLetter = input("Choose a letter and I will find the words that contain that letter regardless of case: ")
for word in wordList:
    if findLetter.upper() in word or findLetter.lower() in word:
        containsLetter.append(word)
